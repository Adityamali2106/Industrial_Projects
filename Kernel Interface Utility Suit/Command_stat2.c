...
/*
	Acccept file name from command line 
...
void DisplayPermission(mode_t m)
{
	...
	if(m & S_IWUSR)	Arr[5] = 'w';
	if(m & S_IXUSR)	Arr[6] = 'x';
...
int main(int argc,char *argv[])
{
	...
	if(access(argv[1],F_OK) == 0)
	{
		...
		if(iRet == -1)
		{
			printf("Error : unable to fatch statistical information \n");
			return -1;
		}
